  ...
  <div>
    ...
    >
      ...
      <template v-slot:item.status="{ item }">
        <v-chip :color="item.status == 5 ? 'orange' : 'green'" dark>{{
          item.status == 5 ? "ongoing" : "completed"
        }}</v-chip>
      </template>
  ...
  computed: {
    formTitle() {
      return this.editedIndex === -1
        ...
        : "Edit " + this.$route.params.transaction_type + " Transaction";
    },
    ...mapState({
