<template>
  <div class="">
    <div class="row">
      ...
      <div class="col">
        <b-input-group prepend="Row " class="pull-right">
          <b-form-select
            :options="perPageoptions"
            v-model="perPage"
            @change="getItem(currentPage)"
          ></b-form-select>
          ...
          <td>
            <b-button-group v-if="branch === hoverSelectedItem">
              <b-button
                ...
                @click="editItem(branch.id)"
                size="sm"
                >Edit</b-button
              >
              <b-button @click="deleteItem(branch.id)" size="sm"
                >Delete</b-button
              >
            </b-button-group>
    ...
    <b-modal id="add-modal" title="Add Record" :hide-footer="true">
      <crud-form
        ...
        :fields="fields"
        :fieldsInit="{ name: '', address: '', enabled: 1 }"
      ></crud-form>
    </b-modal>
    <b-modal id="edit-modal" title="Edit Record" :hide-footer="true">
      <crud-form
        ...
        :modelName="this.$options.name"
        :fields="fields"
        :fieldsInit="{ name: '', address: '', enabled: '' }"
      ></crud-form>
    </b-modal>
    <b-pagination
      v-model="currentPage"
  ...
  methods: {
    ...
    deleteItem(itemId) {
      if (confirm("Do you really want to delete this record?")) {
        axios
          .delete("/" + this.$options.name + "/" + itemId)
          .then((response) => {
