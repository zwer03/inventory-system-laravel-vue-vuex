<template>
    <div class="">
        ...
        <table class="table table-hover mt-3">
            ...
            <tbody>
                <tr v-for="branch in items" :key="branch.id" 
                    @mouseover="onHoverSelectedItem(branch)"
                    @mouseleave="hoverSelectedItem=false">
                    <th scope="row">{{branch.id}} </th>
                    <td>{{branch.name}} </td>
                    <td>{{branch.address}} </td>
                    <td>
                        <b-button-group v-if="branch === hoverSelectedItem">
                            <b-button v-b-modal.edit-modal @click="editItem(branch.id)" size="sm">Edit</b-button>
                            <b-button @click="deleteItem(branch.id)" size="sm">Delete</b-button>
                        </b-button-group>
                        <span v-else >{{branch.created_at}}</span>
                    </td>
                </tr>  
        ...
        <b-modal id="add-modal" title="Add Record" :hide-footer=true>
            <crud-form @completed="addItem" :modelName="this.$options.name" :fields="fields" :fieldsInit="{name: '', address: '', enabled: 1}"></crud-form>
        </b-modal>
        <b-modal id="edit-modal" title="Edit Record" :hide-footer=true>
            <crud-form @updated="updateItem" :id="selectedItem" :modelName="this.$options.name" :fields="fields" :fieldsInit="{name: '', address: '', enabled: ''}"></crud-form>
        </b-modal>
        <b-pagination
            v-model="currentPage"
            v-if="items.length"
