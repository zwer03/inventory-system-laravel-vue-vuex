<template>
    <div>
        <v-container class lg="6" offset-lg="3">
            <v-row v-if="$route.params.transaction_type != 'out' || $route.params.transaction_type != 'transfer'">
                <v-col>
                    <search-warehouse v-if="$route.params.transaction_type" :defaultValue="defaultWarehouse?defaultWarehouse:1" :readOnly="true"></search-warehouse>
                </v-col>
                <v-col>
                    <search-storage @selectedItem="updateSelStorage" :defaultValue="1" :autofocus="true" :readOnly="this.$route.params.status == 10?true:false"></search-storage>
                </v-col>
            </v-row>
            <v-row v-if="$route.params.transaction_type == 'in' || $route.params.transaction_type == 'out'">
                <v-col>
                    <search-company
                ...
                <template v-slot:top>
                    <v-dialog v-model="invDetailsDialog" max-width="500px">
                        <v-card>
                                    ...
                                    <v-row>
                                        ...
                                        <v-col cols="12">
                                            ...
                                            <search-storage
                                                ...
                                                :defaultValue="$route.params.transaction_type == 'transfer'?editedItem.old_storage_id:editedItem.storage_id"
                                                :label="$route.params.transaction_type == 'transfer'?'Get From':'Select Storage'"
                                            ></search-storage>
                                            <search-storage
                            ...
                            <v-card-actions>
                                <v-spacer></v-spacer>
                                <v-btn color="blue darken-1" text @click="invDetailsClose">Cancel</v-btn>
                                <v-btn color="blue darken-1" text @click="invDetailsSave">Update</v-btn>
                            </v-card-actions>
                        </v-card>
                    </v-dialog>
                </template>
                <template v-slot:body.append="{ headers }">
                    <tr>
                        <td :colspan="headers.length">
                            <search-product v-if="$route.params.status != 10" @selectedItem="updateItems" :comboBox="$route.params.transaction_type=='in'?true:false"></search-product>
                        </td>
                    </tr>
                </template>
    ...
    methods: {
        ...
        updateItems(addedItem) {
            const item = {
                ...
                old_storage_id: null,
                location: (!this.storage=='undefined'?this.storage.text:this.storages[0].text),
                storage_id: (!this.storage=='undefined'?this.storage.value:this.storages[0].value),
                quantity: 0,
                unit: "pc"
            };
            if (
                !this.items.some(
                    item =>
                        item.id === addedItem.value &&
                        item.product === addedItem.text
                )
            )
        ...
        invDetailsSave() {
            if (this.editedIndex > -1) {
                if (this.$route.params.transaction_type == "out" || this.$route.params.transaction_type == "transfer") {
                    axios
                        .get(
                        ...
                        .then(response => {
                            ...
                            if (
                                ...
                                this.qtyError = "Unable to proceed. Your current stock in "+this.storage.text+" is " +
                                        response.data.quantity +
